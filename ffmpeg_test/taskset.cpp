...
int taskset_init(taskset_t *ts, int thread_count, size_t arg_size)
{
    int init_step;
    ...
    ts->task_threads = (hb_thread_t**)malloc(sizeof(hb_thread_t*) * ts->thread_count);
    ...
    if (arg_size != 0)
    {
        ts->task_threads_args = (uint8_t *)malloc(arg_size * ts->thread_count);
        if (ts->task_threads == NULL)
            goto fail;
    }
    init_step++;

    ts->task_begin_bitmap = (uint32_t *)malloc(sizeof(uint32_t) * ts->bitmap_elements);
    if (ts->task_begin_bitmap == NULL)
        goto fail;
    init_step++;

    ts->task_complete_bitmap = (uint32_t *)malloc(sizeof(uint32_t) * ts->bitmap_elements);
    if (ts->task_complete_bitmap == NULL)
        goto fail;
    init_step++;

    ts->task_stop_bitmap = (uint32_t *)malloc(sizeof(uint32_t) * ts->bitmap_elements);
...
int taskset_thread_spawn(taskset_t *ts, int thr_idx, char *descr, thread_func_t *func, int priority)
{
    ts->task_threads[thr_idx] = hb_thread_init(descr, func, taskset_thread_args(ts, thr_idx), priority);
    return(ts->task_threads[thr_idx] != NULL);
}

void taskset_cycle(taskset_t *ts)
{
...
*/
void taskset_thread_wait4start(taskset_t *ts, int thr_idx)
{
    hb_lock(ts->task_cond_lock);
...
*/
void taskset_thread_complete(taskset_t *ts, int thr_idx)
{
    hb_lock(ts->task_cond_lock);
...
void taskset_fini(taskset_t *ts)
{
    int i;
